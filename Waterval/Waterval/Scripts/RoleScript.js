$(document).on('click', '#btn_addMod', function () {
    ...
    var naam = '<tr id="' + id + '"><td>' + tablevalue[0] + '<input type="number" id="AccountLaw[' + (index - 1) + '].Law_ID" name="AccountLaw[' + (index - 1) + '].Law_ID" class="form-control hidden" value=' + id + ' /></td>';
    ...
    $('#ModTable').append(naam +  buttonDelete);
            ...
            if ($(this).attr('id') != null) {
                $(this).find("input[type=number]").each(function () {
                    if ($(this).attr('name').match("ID$")) {
                        $(this).attr('name', 'AccountLaw[' + (index) + '].Law_ID')
                        $(this).attr('id', 'AccountLaw[' + (index) + '].Law_ID')
                    }
                });
                index += 1;
            }
