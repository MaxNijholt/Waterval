$(document).on('click', '#btn_addMod', function () {
    ...
    var title = '<tr id="' + id + '"><td>' + tablevalue[0];

    var defi = '<input type="number" id="Study[' + (index - 1) + '].Study_ID" name="Study[' + (index - 1) + '].Study_ID" class="form-control hidden" value=' + id + ' /></td>';
    ...
    $('#ModTable').append(title +defi+ buttonDelete);
            ...
            if ($(this).attr('id') != null) {
                $(this).find("input[type=number]").each(function () {
                    if ($(this).attr('name').match("ID$")) {
                        $(this).attr('name', 'Study[' + (index) + '].Study_ID')
                        $(this).attr('id', 'Study[' + (index) + '].Study_ID')
                    }
                });
                index += 1;
            }
